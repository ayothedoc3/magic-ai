...
class AssistantService
{
    ...
    public function listModels(): array|Collection
    {
        try {
            ...
            // Tam eşleşme ile bloklanacak modeller
            $unsupportedExact = [
                'gpt-5',
                'gpt-image-1',
            ...
            // Alt string / aile bazlı bloklamalar
            // Not: 'preview' genel olarak bloklanmıyor (çünkü gpt-4-1106-preview vb. eğitim destekliyor)
            $blockSubstrings = [
            ...
            $collection = collect($models)->filter(function ($model) use ($unsupportedExact, $blockSubstrings) {
                ...
                // Geçtiyse eğitime uygun kabul ediyoruz
                return true;
            })->values();

            return $collection;
        } catch (Throwable $e) {
            // İstersen burada loglayabilirsin: report($e);
            return [];
        }
    }
